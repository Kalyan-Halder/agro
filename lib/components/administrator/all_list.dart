...
class ManageAllAdminPage extends StatefulWidget {
  final String user_id;

  ManageAllAdminPage({required this.user_id});

  @override
  _ManageAllAdminPageState createState() => _ManageAllAdminPageState(user_id);
}

class _ManageAllAdminPageState extends State<ManageAllAdminPage> {
  ...
  _ManageAllAdminPageState(String user_id) {
    getData(user_id);
  }
  ...
  void _fetchUpdatedProductList() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _ManageAllAdminPageState parentState =
        context.findAncestorStateOfType<_ManageAllAdminPageState>()!;
...
class _ProductListItemState extends State<ProductListItem> {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        title: Text('${widget.product.name} - ${widget.product.quantity} $unit',
            style: widget.product.isVerified ? const TextStyle(color: Colors.black) : const TextStyle(color: Colors.white)
        ),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Category: ${widget.product.category}',style: widget.product.isVerified ? const TextStyle(color: Colors.black) : const TextStyle(color: Colors.white)),
            Text('Price: ${widget.product.price} Taka per $unit',style: widget.product.isVerified ? const TextStyle(color: Colors.black) : const TextStyle(color: Colors.white)),
          ],
        ),
        trailing: Row(
