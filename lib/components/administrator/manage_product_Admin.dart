...
class ManageProductAdminPage extends StatefulWidget {
  final String user_id;

  ManageProductAdminPage({required this.user_id});

  @override
  _ManageProductAdminPageState createState() => _ManageProductAdminPageState(user_id);
}

class _ManageProductAdminPageState extends State<ManageProductAdminPage> {
  ...
  _ManageProductAdminPageState(String user_id) {
    getData(user_id);
  }
  ...
  void getData(String user_id) async {
    ...
    try {
      var response = await http.get(
        Uri.parse(fetch_for_admin_products),
  ...
  void _fetchUpdatedProductList() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _ManageProductAdminPageState parentState =
        context.findAncestorStateOfType<_ManageProductAdminPageState>()!;
...
class _ProductListItemState extends State<ProductListItem> {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        title: Text('${widget.product.name} - ${widget.product.quantity} $unit',style:const TextStyle(color: Colors.white,fontWeight: FontWeight.bold)),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Category: ${widget.product.category}',style:const TextStyle(color: Colors.white)),
            Text('Price: ${widget.product.price} Taka per $unit',style:const TextStyle(color: Colors.white)),
          ],
        ),
        trailing: Row(
